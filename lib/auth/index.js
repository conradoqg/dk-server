...
class Auth {
    ...
    async createUser(email, password) {
        ...
        await this.dbService.users.post(user);
    ...
    async updateUser(user, email, password = null, type = null) {
        ...
        if (resultFound) {
            const user = resultFound.doc;

            if (password != null) {
                if (user.email == email) {
                    if (LENGTH_POLICY.check(password)) user.password = bcrypt.hashSync(password, 8);
            ...
            if (type != null) {
                if (user.type == 'admin') user.type = type;
                else throw new AuthorizationError(`The user '${user.email}' is not authorized to update user '${email}'`);
            }

            await this.dbService.users.put(user);
        } else {
