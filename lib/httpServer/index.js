...
class HTTPServer {
    constructor(dockerService, authService) {
        ...
        this.app.post('/token', asyncHandler(async (req, res) => {
            ...
            if (token == null) throw new HTTPError(401, 'Invalid email and/or password.');
        ...
        this.app.put('/users/:email', [authService.middleware, async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
            ...
            try {
                ...
                if (ex instanceof AuthException) throw new HTTPError(400, `Cannot update user: ${ex.message}`, null, ex);
                else throw ex;
            }
        }]);
