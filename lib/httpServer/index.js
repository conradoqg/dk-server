...
class HTTPServer {
    constructor(dockerService, authService, stackTemplateService, configService, metricService) {
        ...
        this.app.get('/stacks/:stackName/services/:serviceID/logs', authService.createMiddleware(), apiLimiter, async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
            ...
            if (stackName == null) throw new HTTPError(400, 'The stackName parameter must be a string');
            if (serviceID == null) throw new HTTPError(400, 'The serviceID parameter must be a string');

            const logs = await dockerService.getLogsFromServiceFromStack(req.user, stackName, serviceID, req.query.tail);

            if (!logs) throw new HTTPError(404, `Stack '${stackName}' or service '${serviceID}' not found for user '${req.user.name}'.`);
        ...
        this.app.post('/tokens', apiLimiter, async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
