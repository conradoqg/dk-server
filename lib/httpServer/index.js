...
var createUserLimiter = new RateLimit({
...
class HTTPServer {
    constructor(dockerService, authService, stackTemplateService) {
        ...
        this.app.post('/stacks', authService.middleware, asyncHandler(async (req, res) => {
            ...
            const stackTemplatePathFound = await stackTemplateService.getStackTemplatePathByName(composeTemplateName);

            if (!stackTemplatePathFound) throw new HTTPError(400, `Stack template '${composeTemplateName}' not found`);
        ...
        this.app.get('/templates/stacks', authService.middleware, asyncHandler(async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
        ...
        this.app.post('/templates/stacks', authService.middleware, asyncHandler(async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
        ...
        this.app.put('/templates/stacks', authService.middleware, asyncHandler(async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
        ...
        this.app.get('/templates/stacks/:stackTemplateName', authService.middleware, asyncHandler(async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
            ...
            if (stackTemplate == null) throw new HTTPError(404, `Template '${stackTemplateName} not found`);

            res.status(200).json(stackTemplate);
        }));

        this.app.delete('/templates/stacks/:stackTemplateName', authService.middleware, asyncHandler(async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
            ...
            try {
                ...
                if (!stackTemplateDeleted) throw new HTTPError(404, `Template '${stackTemplateName} not found`);
        ...
        this.app.post('/users', (process.env.ENV == 'development' ? (req, res, next) => next() : createUserLimiter), asyncHandler(async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
        ...
        this.app.put('/users', authService.middleware, async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
            ...
            try {
                ...
                res.status(200).json(projection.userUpdationResult(userUpdated));
            } catch (ex) {
                if (ex instanceof InvalidOperationError) throw new HTTPError(400, ex.message, null, ex);
                if (ex instanceof AuthorizationError) throw new HTTPError(403, ex.message, null, ex);
                else throw ex;
