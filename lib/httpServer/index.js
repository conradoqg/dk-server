...
const swaggerDocument = yaml.safeLoad(require('fs').readFileSync(require('path').join(__dirname, './swagger.yml')));
...
class HTTPServer {
    constructor(dockerService, authService, stackTemplateService) {
        ...
        this.app.get('/stacks', authService.middleware, async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
        ...
        this.app.post('/stacks', authService.middleware, async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
        ...
        this.app.get('/stacks/:stackName', authService.middleware, async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
        ...
        this.app.delete('/stacks/:stackName', authService.middleware, async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
        ...
        this.app.get('/templates/stacks', authService.middleware, async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
        ...
        this.app.post('/templates/stacks', authService.middleware, async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
        ...
        this.app.put('/templates/stacks', authService.middleware, async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
        ...
        this.app.get('/templates/stacks/:stackTemplateName', authService.middleware, async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
        ...
        this.app.delete('/templates/stacks/:stackTemplateName', authService.middleware, async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
        ...
        this.app.get('/users', (process.env.NODE_ENV == 'development' ? (req, res, next) => next() : createUserLimiter), async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
        ...
        this.app.post('/users', (process.env.NODE_ENV == 'development' ? (req, res, next) => next() : createUserLimiter), async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
