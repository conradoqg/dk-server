...
class HTTPServer {
    constructor(dockerService, authService, stackTemplateService) {
        ...
        this.app.get('/users', (process.env.ENV == 'development' ? (req, res, next) => next() : createUserLimiter), async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
        ...
        this.app.post('/users', (process.env.ENV == 'development' ? (req, res, next) => next() : createUserLimiter), async (req, res) => {
            if (req.headers['accept'] && !req.headers['accept'].includes('application/json')) throw new HTTPError(400, 'Accept content not supported');
