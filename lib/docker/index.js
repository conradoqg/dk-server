...
class Docker {
        ...
        if (user.type == 'admin' || (user.type == 'user' && userStacks.length <= this.maxStacksPerUser)) {
            ...
            if (doc.services) {
                Object.values(doc.services).map(service => {
                    ...
                    service.deploy.labels.name = user.name;
            ...
            throw new InvalidOperationError(`Max stacks for user '${user.name}' reached (${this.maxStacksPerUser})`);
        }
    ...
    async getServices(user, stackName = null) {
        ...
        if (user.type != 'admin') labels.push(`name=${user.name}`);
    ...
    async removeStackByName(user, stackName) {
        ...
        const foundStack = stack.Services.find(service => service.Spec && service.Spec.Labels && service.Spec.Labels['com.docker.stack.namespace'] == stackName && (user.type == 'admin' || service.Spec.Labels['name'] == user.name));
    ...
    async getLogsFromServiceFromStack(user, stackName, serviceID, tail = 'all') {
        // Check if the user can see this stack
        const stack = await this.getStackByName(user, stackName);

        if (stack != null) {
            // Check if the service request is from one of the user's stack
            const service = stack.Services.map(service => service.ID == serviceID);

            if (service) {
                const dockerService = await this.dockerAPI.getService(serviceID);
                const log = await dockerService.logs({ 
                    tail,
                    stdout: true,
                    stderr: true
                });
